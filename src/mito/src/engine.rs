...
pub(crate) const INIT_TABLE_VERSION: TableVersion = 0;
...
impl<S: StorageEngine> MitoEngineInner<S> {
    ...
    async fn open_table(
        ...
        let table = {
            ...
            let Some(table) = MitoTable::open(table_name, &table_dir, region, self.object_store.clone())
                .await
                .map_err(BoxedError::new)
                .context(TableOperationSnafu)? else {
                    return Ok(None);
                };
...
mod tests {
    ...
    async fn test_alter_rename_table() {
        let (engine, table_engine, table, object_store, _dir) =
            test_util::setup_mock_engine_and_table().await;
        let ctx = EngineContext::default();
        ...
        let altered_table = table_engine.alter_table(&ctx, req).await.unwrap();
        assert_eq!(altered_table.table_info().name, new_table_name);
        ...
        let table_renamed = table_engine
            ...
            .unwrap()
            .unwrap();
        assert_eq!(table_renamed.table_info(), table.table_info());
    }
