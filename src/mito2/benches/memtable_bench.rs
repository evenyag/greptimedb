...
use mito2::memtable::bulk::part_reader::{RecordBatchIter, NonEncodedRecordBatchIter};
    ...
    for &rows in &[1024, 2048, 4096, 8192] {
        group.bench_with_input(format!("{}_rows", rows), &rows, |b, &rows| {
            b.iter(|| {
                ...
                // Convert to BulkPart
                let (_bulk_part, _) = converter.convert().unwrap();
            });
        });
    }
}
...
fn record_batch_iter(c: &mut Criterion) {
    ...
    group.bench_function("4096_rows_4096_hosts", |b| {
        b.iter(|| {
            ...
            let codec = Arc::new(DensePrimaryKeyCodec::new(&metadata));
            let mut converter = BulkPartConverter::new(&metadata, 4096, codec, false);
...
fn bulk_part_converter_with_pk_columns(c: &mut Criterion) {
    ...
    for &rows in &[1024, 2048, 4096, 8192] {
        ...
        group.bench_with_input(format!("{}_rows_with_pk_columns", rows), &rows, |b, &rows| {
            b.iter(|| {
                let generator =
                    CpuDataGenerator::new(metadata.clone(), rows, start_sec, start_sec + 1);
                let codec = Arc::new(DensePrimaryKeyCodec::new(&metadata));
...
fn record_batch_iter_filter(c: &mut Criterion) {
    let metadata = Arc::new(cpu_metadata());
    let start_sec = 1710043200;

    let mut group = c.benchmark_group("record_batch_iter_filter");

    // Pre-create RecordBatch for filtering benchmark
    ...
    // Benchmark with hostname filter
    group.bench_function("4096_rows_with_hostname_filter", |b| {
        b.iter(|| {
            // Create context for RecordBatchIter with predicate
            let context = Arc::new(BulkIterContext::new(
                metadata.clone(),
                &None,                // No projection
            ...
            let mut iter = RecordBatchIter::new(record_batch_with_filter.clone(), context, None);
    ...
    group.bench_function("4096_rows_no_filter", |b| {
        b.iter(|| {
            // Create context for RecordBatchIter without predicate
            let context = Arc::new(BulkIterContext::new(
                metadata.clone(),
                &None,  // No projection
                None,   // No predicate
            ...
            // Create and iterate over RecordBatchIter
            let mut iter = RecordBatchIter::new(record_batch_no_filter.clone(), context, None);
...
fn non_encoded_record_batch_iter_filter(c: &mut Criterion) {
    let metadata = Arc::new(cpu_metadata());
    let start_sec = 1710043200;

    let mut group = c.benchmark_group("non_encoded_record_batch_iter_filter");

    // Pre-create RecordBatch and primary key arrays
    let (record_batch_with_filter, primary_key_arrays_with_filter, record_batch_no_filter, primary_key_arrays_no_filter) = {
        let generator = CpuDataGenerator::new(metadata.clone(), 4096, start_sec, start_sec + 1);
        let codec = Arc::new(DensePrimaryKeyCodec::new(&metadata));
        let mut converter = BulkPartConverter::new(&metadata, 4096, codec, true);
        ...
        let (bulk_part, primary_key_arrays) = converter.convert().unwrap();
        let record_batch = bulk_part.batch;
        
        (record_batch.clone(), primary_key_arrays.clone(), record_batch, primary_key_arrays)
    };
    ...
    group.bench_function("4096_rows_with_hostname_filter", |b| {
        b.iter(|| {
            // Create context for NonEncodedRecordBatchIter with predicate
            let context = Arc::new(BulkIterContext::new(
                metadata.clone(),
                &None,                // No projection
                Some(predicate.clone()), // With hostname filter
            ...
            // Create and iterate over NonEncodedRecordBatchIter with filter
            let mut iter = NonEncodedRecordBatchIter::new(
                record_batch_with_filter.clone(), 
                primary_key_arrays_with_filter.clone(), 
                context, 
                None
            );
    ...
    group.bench_function("4096_rows_no_filter", |b| {
        b.iter(|| {
            // Create context for NonEncodedRecordBatchIter without predicate
            let context = Arc::new(BulkIterContext::new(
                metadata.clone(),
                &None,  // No projection
                None,   // No predicate
            ...
            // Create and iterate over NonEncodedRecordBatchIter
            let mut iter = NonEncodedRecordBatchIter::new(
                record_batch_no_filter.clone(), 
                primary_key_arrays_no_filter.clone(), 
                context, 
                None
            );
...
criterion_group!(
    ...
    record_batch_iter_filter,
    non_encoded_record_batch_iter_filter
);
