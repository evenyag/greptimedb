...
/// Cached rows selected by a time-series selector.
pub enum SelectorResult {
    /// Cached rows in primary-key `Batch` format.
    PrimaryKey(Vec<Batch>),
    /// Cached rows in flat `RecordBatch` format.
...
impl SelectorResultValue {
    /// Creates a new selector result value in primary-key format.
    ...
    /// Creates a new selector result value in flat format.
