...
/// Scans file ranges at `index`.
pub(crate) fn scan_file_ranges_plain(
    stream_ctx: Arc<StreamContext>,
    part_metrics: PartitionMetrics,
    index: RowGroupIndex,
    read_type: &'static str,
    ...
    try_stream! {
        ...
        for range in ranges {
            let build_reader_start = Instant::now();
            let mut reader = range.plain_reader().await?;
            let build_cost = build_reader_start.elapsed();
            part_metrics.inc_build_reader_cost(build_cost);
            let compat_batch = range.compat_batch().map(|compat| compat.as_plain());
            while let Some(mut batch) = reader.next_batch().await? {
                if let Some(compact_batch) = compat_batch {
                    batch = compact_batch.compat(batch)?;
                }
                yield batch;
            }
            let prune_metrics = reader.metrics();
