...
impl Batch {
    ...
    /// Extends primary key values.
    pub fn extend_pk_values(&mut self, values: &[(ColumnId, Value)]) {
        if let Some(pk_values) = &mut self.pk_values {
    ...
    pub(crate) fn take_in_place(&mut self, indices: &UInt32Vector) -> Result<()> {
