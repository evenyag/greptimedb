...
impl SeriesScan {
    ...
    pub(crate) fn new(input: ScanInput) -> Self {
        Self::new_with_mode(input, SeriesScanMode::Legacy)
    }

    /// Creates an experimental two-stage series scan.
    pub(crate) fn new_two_stage(input: ScanInput) -> Self {
        Self::new_with_mode(input, SeriesScanMode::TwoStage)
    }

    fn new_with_mode(input: ScanInput, mode: SeriesScanMode) -> Self {
        ...
        let pruner = if mode == SeriesScanMode::TwoStage {
            // The value-reading phase reuses file range builders created while
            // discovering candidate series.
            Arc::new(Pruner::new_with_retained_builders(
                stream_ctx.clone(),
                num_workers,
                true,
            ))
        } else {
            Arc::new(Pruner::new(stream_ctx.clone(), num_workers))
        };
