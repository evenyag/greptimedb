...
impl Pruner {
    ...
    async fn prune_file_directly(
        ...
        // Retained mode caches until the pruner is dropped. Otherwise, cache only
        // while the file still has remaining ranges.
        {
            let mut entry = self.inner.file_entries[file_index].lock().unwrap();
        ...
        while let Some(request) = rx.recv().await {
            ...
            match result {
                Ok(builder) => {
                    ...
                    // Retained mode caches until the pruner is dropped. Otherwise, if
...
fn should_cache_builder(entry: &FileBuilderEntry, retain_builders: bool) -> bool {
    entry.builder.is_none() && (retain_builders || entry.remaining_ranges > 0)
}

/// Caches a freshly pruned builder if retained or still referenced, and records
/// the corresponding builder memory/count deltas for verbose metrics.
fn cache_builder_if_needed(
    ...
    if should_cache_builder(entry, retain_builders) {
        reader_metrics.metadata_mem_size += builder.memory_size() as isize;
        reader_metrics.num_range_builders += 1;
...
mod tests {
    ...
    fn should_cache_builder_when_ranges_remain() {
        ...
        assert!(should_cache_builder(&entry, false));
    }
    ...
    fn should_not_cache_builder_when_no_ranges_remain() {
        ...
        assert!(!should_cache_builder(&entry, false));
        assert!(should_cache_builder(&entry, true));
    }
    ...
    async fn worker_does_not_cache_after_skip_file_range_consumed_all() {
        ...
        let should_cache = should_cache_builder(&entry, false);
        drop(entry);
