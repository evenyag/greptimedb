...
impl SeqScan {
    ...
    ///
    /// Checks the range cache before building sources. On a cache hit, replays
    /// the cached batches. On a miss, builds sources normally and wraps the
    /// resulting stream to populate the cache for future hits.
