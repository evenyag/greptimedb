...
use crate::read::scan_util::{
    scan_file_ranges, scan_file_ranges_multi_series, scan_file_ranges_plain, scan_mem_ranges,
...
impl UnorderedScan {
        ...
        let stream = try_stream! {
            ...
            for part_range in part_ranges {
                ...
                for await batch in stream {
                    ...
                    let convert_start = Instant::now();
                    let record_batch = stream_ctx.input.mapper.as_primary_key().convert_multi(&batch)?;
                    metrics.convert_cost += convert_start.elapsed();
                    let yield_start = Instant::now();
                    yield record_batch;
                    metrics.yield_cost += yield_start.elapsed();
    ...
    fn scan_partition_plain(
        ...
        let stream = try_stream! {
            ...
            for part_range in part_ranges {
                ...
                let stream = Self::scan_partition_range_plain(
                    stream_ctx.clone(),
                    part_range.identifier,
                    part_metrics.clone(),
                    range_builder_list.clone(),
    ...
    /// Scans a [PartitionRange] by its `identifier` and returns a stream.
    fn scan_partition_range_plain(
        stream_ctx: Arc<StreamContext>,
        part_range_id: usize,
        part_metrics: PartitionMetrics,
        range_builder_list: Arc<RangeBuilderList>,
    ) -> impl Stream<Item = Result<PlainBatch>> {
        stream! {
            // Gets range meta.
            let range_meta = &stream_ctx.ranges[part_range_id];
            for index in &range_meta.row_group_indices {
