...
mod tests {
    ...
    fn create_test_record_batch(
        ...
        let mut builder = BinaryDictionaryBuilder::<datatypes::arrow::datatypes::UInt32Type>::new();
    ...
    /// A test iterator that wraps a RecordBatch.
    struct TestRecordBatchIterator {
        batches: Vec<RecordBatch>,
        index: usize,
    }

    impl TestRecordBatchIterator {
        fn new(batches: Vec<RecordBatch>) -> Self {
            Self { batches, index: 0 }
        }
    }

    impl Iterator for TestRecordBatchIterator {
    ...
    fn test_merge_iterator_empty() {
        ...
        let mut merge_iter = MergeIterator::new(schema, vec![], 1, 1024).unwrap();
    ...
    fn test_merge_iterator_single_batch() {
        ...
        let iter = Box::new(TestRecordBatchIterator::new(vec![batch.clone()]));

        let merge_iter = MergeIterator::new(schema, vec![iter], 1, 1024).unwrap();
    ...
    fn test_merge_iterator_non_overlapping() {
        ...
        let iter1 = Box::new(TestRecordBatchIterator::new(vec![
            batch1.clone(),
            batch3.clone(),
        ]));
        let iter2 = Box::new(TestRecordBatchIterator::new(vec![batch2.clone()]));

        let merge_iter = MergeIterator::new(schema, vec![iter1, iter2], 1, 1024).unwrap();
        let result = collect_merge_iterator_batches(merge_iter).unwrap();
    ...
    fn test_merge_iterator_overlapping_timestamps() {
        ...
        let schema = batch1.schema();
        let iter1 = Box::new(TestRecordBatchIterator::new(vec![batch1]));
        let iter2 = Box::new(TestRecordBatchIterator::new(vec![batch2]));
    ...
    fn test_batch_builder_basic() {
        ...
        let result = builder.build_record_batch().unwrap();
    ...
    fn test_row_cursor_comparison() {
        ...
        let columns1 = SortColumns::new(&batch1, 1);
        let columns2 = SortColumns::new(&batch2, 1);
