...
impl FileRange {
    ...
    /// Returns a multi series reader to read the [FileRange].
    pub(crate) async fn multi_series_reader(&self) -> Result<MultiSeriesPruneReader> {
        let parquet_reader = self
            .context
            .reader_builder
            .build(self.row_group_idx, self.row_selection.clone())
        ...
        for filter_ctx in &self.filters {
            ...
            let result = match filter_ctx.semantic_type() {
                ...
                SemanticType::Field => {
                    ...
                    filter.evaluate_array(field_col).context(RecordBatchSnafu)?
                }
                SemanticType::Timestamp => filter
