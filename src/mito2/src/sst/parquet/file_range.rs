...
impl FileRange {
    ...
    /// Returns a flat reader to read the [FileRange] that returns RecordBatch.
    /// Currently it doesn't support the RowGroupLastRowCachedReader.
...
impl FileRangeContext {
    ...
    /// TRY THE BEST to perform pushed down predicate precisely on the input RecordBatch.
    /// Return the filtered RecordBatch. If the entire batch is filtered out, return None.
    /// Used by FlatPruneReader for flat format filtering.
...
impl RangeBase {
    ...
    /// TRY THE BEST to perform pushed down predicate precisely on the input RecordBatch.
    /// Return the filtered RecordBatch. If the entire batch is filtered out, return None.
    /// This method is similar to precise_filter but works with RecordBatch instead of Batch.
    pub(crate) fn precise_filter_flat(&self, input: RecordBatch) -> Result<Option<RecordBatch>> {
        let mut mask = BooleanBuffer::new_set(input.num_rows());
