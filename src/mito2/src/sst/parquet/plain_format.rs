...
//!
//! We store two addtional internal columns in parquet:
//! - `__sequence`, the sequence number of a row. Type: uint64
//! - `__op_type`, the op type of the row. Type: uint8
//!
//! Internal columns are stored at the end of the schema.
//! We other columns in the same order as [RegionMetadata::field_columns()](store_api::metadata::RegionMetadata::field_columns()).
//!
...
impl PlainReadFormat {
    ...
    //     ensure!(
    //         record_batch.num_columns() >= PLAIN_FIXED_POS_COLUMN_NUM,
    ...
    //         .iter()
    //         .rev()
    //         .take(PLAIN_FIXED_POS_COLUMN_NUM);
    ...
    //         .take(record_batch.num_columns() - PLAIN_FIXED_POS_COLUMN_NUM) // Take all field columns.
    //         .map(|(array, field)| {
    ...
    //     self.arrow_schema.fields.len() - PLAIN_FIXED_POS_COLUMN_NUM
    // }
...
pub(crate) fn parquet_row_group_time_range(
    ...
    assert!(
        num_columns >= PLAIN_FIXED_POS_COLUMN_NUM,
        "file only has {} columns",
        num_columns
    );
    let time_index_pos = num_columns - PLAIN_FIXED_POS_COLUMN_NUM;
