...
use arrow1::array::{Array, ArrayRef};
use arrow1::bitmap::Bitmap;
use snafu::ensure;
...
use crate::vectors2::operations::VectorOp;
    ...
    ($Array: ident, $Vector: ident) => {
        impl $Vector {
            pub fn try_from_arrow_array(
                array: impl AsRef<dyn arrow1::array::Array>,
...
macro_rules! impl_extend_for_builder {
    ($mutable_vector: expr, $vector: ident, $VectorType: ident, $offset: ident, $length: ident) => {{
        ...
        for value in concrete_vector.iter_data() {
            // match value {
            //     Some(v) => $mutable_array.append_value(v),
            //     None => $mutable_array.append_null(),
            // }
