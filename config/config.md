...
| `region_engine.mito.max_background_index_builds` | Integer | Auto | Max number of running background index jobs (default: 1/8 of cpu cores). |
| `region_engine.mito.experimental_series_index_root` | String | `""` | Root below `data_home` for range and series indexes, currently stored locally.<br/>An empty value disables background series-index maintenance.<br/>Under development; do not enable this feature. |
| `region_engine.mito.experimental_series_index_reconcile_interval` | String | `5m` | Interval between series index reconciliation passes.<br/>Under development; do not enable this feature. |
| `region_engine.mito.experimental_series_index_bucket_width` | String | `5d` | Requested minimum event-time bucket width for series indexes. The effective width is<br/>rounded up to an exact multiple of each region's compaction time window.<br/>Under development; do not enable this feature. |
