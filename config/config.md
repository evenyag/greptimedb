...
| `region_engine.mito.series_key_batch_size` | Integer | `500` | Number of series keys in each per-series key distribution batch. |
...
| `region_engine.mito.experimental_series_key_scan` | Bool | `false` | Whether to enable the experimental per-series key scan path. |
| `region_engine.mito.experimental_series_key_scan_use_range_index` | Bool | `false` | Whether the experimental per-series key scan path may use cached per-SST range indexes to build row selections. |
| `region_engine.mito.experimental_series_key_scan_use_streaming_merge` | Bool | `false` | Whether the experimental per-series key scan path may merge sorted per-SST primary-key streams. |
